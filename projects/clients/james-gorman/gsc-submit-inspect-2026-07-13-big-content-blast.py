...
REPORT_JSON = BASE / "gsc-indexing-2026-07-13-big-content-blast.json"
REPORT_MD = BASE / "gsc-indexing-2026-07-13-big-content-blast.md"
PUBLISH_REPORT = BASE / "blog-publish-report-2026-07-13-big-content-blast.json"
...
def load_tokens():
    ...
    for key in ["GSC_CLIENT_ID", "GSC_CLIENT_SECRET", "GSC_REFRESH_TOKEN", "GSC_ACCESS_TOKEN"]:
        if os.environ.get(key):
            data[key.removeprefix("GSC_").lower()] = os.environ[key]
    return data
...
def refresh() -> str:
    ...
    with urllib.request.urlopen(req, timeout=30) as response:
        new = json.loads(response.read().decode())
...
def main():
    ...
    lines = [
        "# GSC Indexing - James Gorman Big Content Blast - 13 July 2026",
        "",
        f"- Property: `{SITE}`",
        ...
        json.dumps(
            {
                ...
                "errors": [row for row in out["inspections"] if not row.get("ok")],
