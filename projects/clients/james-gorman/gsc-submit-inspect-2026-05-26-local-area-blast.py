...
REPORT_JSON = BASE / 'gsc-indexing-2026-05-26-local-area-blast.json'
REPORT_MD = BASE / 'gsc-indexing-2026-05-26-local-area-blast.md'
PUBLISH_REPORT = BASE / 'blog-publish-report-2026-05-26-local-area-blast.json'
...
def load_tokens():
    ...
    # Prefer process env when a fresh GSC token bundle has been sourced.
...
def main():
    ...
    for url in urls:
        ...
        out['inspections'].append({'url': url, 'ok': res.get('ok'), 'status': res.get('status'), 'verdict': verdict, 'coverageState': coverage, 'raw': res})
    REPORT_JSON.write_text(json.dumps(out, indent=2))
    lines = ['# GSC Indexing - James Gorman Local Area Blast - 26 May 2026', '', f"- Property: `{SITE}`", f"- Sitemap submitted: `{SITEMAP}`", f"- Sitemap submit status: {out['sitemap_submit'].get('status')} / ok={out['sitemap_submit'].get('ok')}", '', '## URL Inspection']
    for row in out['inspections']:
        lines.append(f"- {row['url']} — ok={row['ok']} status={row['status']} verdict={row.get('verdict')} coverage={row.get('coverageState')}")
    REPORT_MD.write_text('\n'.join(lines) + '\n')
    print(json.dumps({'sitemap_ok': out['sitemap_submit'].get('ok'), 'inspected': len(out['inspections']), 'errors': [r for r in out['inspections'] if not r.get('ok')], 'report': str(REPORT_MD)}, indent=2))
