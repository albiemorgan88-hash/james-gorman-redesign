...
const originalPublishDate = '2026-06-16';
...
const authority = {
  ...
  govCinealtas: ['Department of Education Ireland: Cineáltas', 'https://www.gov.ie/en/department-of-education/publications/cine%C3%A1ltas-action-plan-on-bullying/'],
...
function header() {
  return `<a class="skip-link" href="#main-content">Skip to main content</a><header class="site-header"><nav class="nav" aria-label="Primary navigation"><a class="brand" href="/"><span class="mark">HiP</span><span>HIP Psychology</span></a><button class="nav-toggle" type="button" aria-expanded="false" aria-controls="primary-nav"><span class="nav-toggle-label">Menu</span></button><div class="nav-links" id="primary-nav">${navHtml()}</div></nav></header>`;
}

function footer() {
  return `<footer class="footer"><div class="footer-inner"><div>HIP Psychology anti-bullying support for NI and ROI schools.</div><div><a href="/blog/">Blog</a><a href="/resources/">Resources</a><a href="/workshops/anti-bullying/">Workshops</a><a href="/support-and-safeguarding/">Support and safeguarding</a><a href="/privacy/">Privacy</a><a href="/sitemap.xml">Sitemap</a></div></div></footer>`;
}

function supportPanel() {
  return `<aside class="support-panel" aria-labelledby="support-panel-title"><div><p class="eyebrow">Support</p><h2 id="support-panel-title">Need help with bullying now?</h2><p>If you are a pupil or parent, contact your school’s safeguarding or pastoral team or speak to a trusted adult. If someone is in immediate danger, call 999 or 112.</p></div><div class="support-links"><a href="https://www.childline.org.uk/get-support/">Childline UK</a><a href="https://www.childline.ie/topics/bullying-and-abuse">Childline Ireland</a><a href="/support-and-safeguarding/">More support information</a></div></aside>`;
}
...
function articleSchema(page) {
  return { '@context': 'https://schema.org', '@type': 'Article', headline: page.h1, description: page.description, author: { '@type': 'Organization', name: 'HIP Psychology' }, publisher: { '@type': 'Organization', name: 'HIP Psychology' }, datePublished: page.datePublished ?? originalPublishDate, dateModified: page.dateModified ?? today, mainEntityOfPage: `${site}${page.slug}` };
}

function blogPostingSchema(page) {
  return { '@context': 'https://schema.org', '@type': 'BlogPosting', headline: page.h1, description: page.description, author: { '@type': 'Organization', name: 'HIP Psychology' }, publisher: { '@type': 'Organization', name: 'HIP Psychology' }, datePublished: page.datePublished ?? originalPublishDate, dateModified: page.dateModified ?? today, mainEntityOfPage: `${site}${page.slug}` };
}
...
function serviceSchema(page) {
  return { '@context': 'https://schema.org', '@type': 'Service', name: page.h1, serviceType: 'School wellbeing workshops', provider: { '@type': 'Organization', name: 'HIP Psychology' }, areaServed: ['Northern Ireland', 'Republic of Ireland'], url: `${site}${page.slug}` };
}
...
function ctaBlock(kind = 'pack') {
  ...
  return `<section class="section"><div class="cta-band"><div><h2>Explore the Anti-Bullying Week classroom pack</h2><p>See the planned assembly scripts, lesson plans, Odd Socks Day printables and parent newsletter support for Anti-Bullying Week 2026.</p></div><a class="button" href="/resources/free-anti-bullying-classroom-pack/">See what is included</a></div></section>`;
}

function formStatus() {
  return '<p class="form-status" role="status" aria-live="polite" tabindex="-1"></p>';
}

function leadForm(pack) {
  return `<form class="lead-form js-enquiry-form" action="/api/enquiry" method="post"><input type="hidden" name="requestType" value="resource"><input type="hidden" name="resource" value="${esc(pack)}"><div class="honeypot" aria-hidden="true"><label>Leave this field empty<input name="website" tabindex="-1" autocomplete="off"></label></div><label>Name<input name="name" autocomplete="name" required></label><label>School or organisation<input name="school" autocomplete="organization" required></label><label>Role<select name="role" required><option value="">Choose your role</option><option>Principal / Vice-Principal</option><option>Pastoral Care / Safeguarding Lead</option><option>Wellbeing Coordinator</option><option>Teacher</option><option>Parent / Carer</option><option>Other</option></select></label><label>Work email<input name="email" type="email" autocomplete="email" required></label><label class="consent"><input name="privacyAcknowledged" type="checkbox" value="yes" required><span>I have read the <a href="/privacy/">privacy notice</a> and understand how this request will be handled.</span></label><button class="button" type="submit">Ask for this resource</button><p class="form-note">Please do not include confidential information about an individual pupil.</p>${formStatus()}</form>`;
}

function workshopForm() {
  return `<form class="lead-form workshop-form js-enquiry-form" action="/api/enquiry" method="post"><input type="hidden" name="requestType" value="workshop"><div class="honeypot" aria-hidden="true"><label>Leave this field empty<input name="website" tabindex="-1" autocomplete="off"></label></div><div class="form-grid"><label>Name<input name="name" autocomplete="name" required></label><label>Work email<input name="email" type="email" autocomplete="email" required></label><label>School or organisation<input name="school" autocomplete="organization" required></label><label>Role<input name="role" autocomplete="organization-title" required></label><label>Location<select name="location" required><option value="">Choose a location</option><option>Northern Ireland</option><option>Republic of Ireland</option><option>Other</option></select></label><label>Workshop group<select name="audience" required><option value="">Choose a group</option><option>Primary pupils</option><option>Post-primary pupils</option><option>Staff team</option><option>Whole-school programme</option><option>Not sure yet</option></select></label><label>Approximate group size<input name="groupSize" inputmode="numeric"></label><label>Preferred term or date<input name="timing"></label></div><label>What would you like the workshop to support?<textarea name="message" rows="5"></textarea></label><label class="consent"><input name="privacyAcknowledged" type="checkbox" value="yes" required><span>I have read the <a href="/privacy/">privacy notice</a> and understand how this enquiry will be handled.</span></label><button class="button" type="submit">Request a workshop quote</button><p class="form-note">Please do not include confidential information about an individual pupil. An enquiry does not commit your school to a booking.</p>${formStatus()}</form>`;
}
...
function redirectPage(destination, label) {
  const title = `Page moved: ${label}`;
  return `<!doctype html>
<html lang="en-GB">
<head>
  <meta charset="utf-8">
  ...
  <meta name="description" content="This page has moved to ${esc(label)}.">
  <meta http-equiv="refresh" content="0;url=${destination}">
  <link rel="canonical" href="${site}${destination}">
  <title>${esc(title)} | HIP Psychology</title>
</head>
<body>
  <main><h1>Page moved</h1><p>This page has moved to <a href="${destination}">${label}</a>.</p></main>
</body>
</html>
`;
}
...
const home = {
  ...
  description: 'Anti-bullying support for schools across NI and Ireland, with Anti-Bullying Week planning, policy resources and workshop options.',
  h1: 'Anti-bullying support for schools across Northern Ireland and Ireland',
};

function homePage() {
  const body = `<section class="hero"><div><p class="eyebrow">ANTI-BULLYING WEEK 2026</p><h1>${home.h1}</h1><p class="lead">Practical planning guides and school workshop support for Anti-Bullying Week 2026 (16-20 November), written for primary, post-primary and staff teams across NI and ROI.</p><div class="hero-actions"><a class="button" href="/workshops/anti-bullying/">Request a workshop quote</a><a class="button secondary" href="/resources/free-anti-bullying-classroom-pack/">Explore classroom resources</a></div><p class="trust-strip">Built for principals, pastoral and safeguarding leads, wellbeing teams and teachers.</p></div><aside class="poster-card" aria-label="HIP Psychology anti-bullying support overview"><div class="poster-top"><div class="poster-brand"><span class="mark">HiP</span><div><strong>HIP Psychology</strong><span>PSYCHOLOGY LTD</span></div></div><div class="badge">Anti-Bullying NI</div></div><h2>Workshops for <span>kindness, respect and resilience</span></h2><p>Practical classroom guidance and school workshop options for Anti-Bullying Week 2026.</p><img class="poster-image" src="/classroom.jpg" alt="School wellbeing classroom workshop for Anti-Bullying Week" loading="eager"><div class="quick-links"><a href="/anti-bullying-week/2026/">Anti-Bullying Week 2026 guide <span>Read</span></a><a href="/anti-bullying-week/odd-socks-day-2026/">Odd Socks Day 2026 <span>Plan</span></a><a href="/resources/free-anti-bullying-classroom-pack/">Classroom pack details <span>View</span></a><a href="/workshops/anti-bullying/">Workshop options <span>View</span></a></div></aside></section><section class="section tight"><h2>Resources for Anti-Bullying Week</h2><div class="grid">${card('Anti-Bullying Week 2026 guide', 'Dates, Break the Silence theme notes, daily activities, assembly scripts and lesson plans for primary and post-primary classrooms.', '/anti-bullying-week/2026/', 'Read the full guide')}${card('Odd Socks Day 2026 ideas', 'Assembly prompts, classroom activities and parent-message copy for schools marking Odd Socks Day on Monday 16 November.', '/anti-bullying-week/odd-socks-day-2026/', 'Plan Odd Socks Day')}${card('Classroom pack', 'See the planned lesson, assembly, printable and parent-communication materials for primary and post-primary settings.', '/resources/free-anti-bullying-classroom-pack/', 'See what is included')}${card('Policy review checklists', 'Explore checklist routes aligned to the Addressing Bullying in Schools Act (NI) 2016 and Bí Cineálta procedures for Irish schools.', '/resources/anti-bullying-policy-checklist/', 'Explore the checklists')}</div></section><section class="section"><h2>Support for every school team</h2><div class="grid">${card('Anti-bullying programme support', 'Build a whole-school programme that connects awareness activity, pupil voice, reporting routes, policy review and staff confidence.', '/anti-bullying/', 'Explore programme support')}${card('Northern Ireland schools', 'Guidance around the Addressing Bullying in Schools Act, Education Authority resources, policy review and whole-school prevention.', '/northern-ireland-schools/', 'Explore NI school support')}${card('Republic of Ireland schools', 'Practical context around Cineáltas, Bí Cineálta procedures, school culture and prevention in primary and post-primary settings.', '/republic-of-ireland-schools/', 'Explore ROI school support')}${card('Cyberbullying and online safety', 'Age-appropriate support for group chats, online reporting, bystander behaviour, social platforms and pupil wellbeing.', '/cyberbullying-in-schools/', 'Explore cyberbullying support')}</div></section>${ctaBlock('workshop')}`;
...
const pages = [
  ...
  },
  {
    ...
    sections: [
      ...
      { id: 'downloads', title: 'Resources and workshops', html: '<p>Explore the classroom pack for planned assembly scripts, lesson plans and Odd Socks Day material. Schools that want live delivery can enquire about HIP Psychology workshops for primary, post-primary or staff teams.</p><p><a href="/resources/free-anti-bullying-classroom-pack/">See the classroom pack details</a> or <a href="/workshops/anti-bullying/">view anti-bullying workshops</a>.</p>' },
    ],
  ...
  },
  {
    ...
    faqs: [['Who is this page for?', 'It is for NI principals, pastoral leaders, anti-bullying coordinators, teachers and wellbeing staff.'], ['Does this replace legal guidance?', 'No. It supports school planning and links to official sources, but schools should still use statutory and Education Authority guidance.'], ['Can HIP Psychology deliver workshops in NI schools?', 'Yes. The workshop page sets out primary, post-primary and staff options.'], ['Which policy resource is designed for NI schools?', 'The NI Anti-Bullying Policy Checklist is the region-specific resource.']],
  },
  {
  ...
  },
  {
    ...
    sections: [
      ...
      { id: 'checklist', title: 'Use the NI checklist', html: '<p>The NI checklist brings review dates, recording prompts, consultation questions and a clear policy audit structure into one practical resource.</p><p><a href="/resources/anti-bullying-policy-checklist-ni/">Open the NI Anti-Bullying Policy Checklist page</a> or compare it with the <a href="/resources/anti-bullying-policy-checklist/">cross-border policy checklist</a>.</p>' },
    ],
    faqs: [['Can schools use a generic template?', 'A generic template is only useful if it is adapted to the school context and checked against current NI requirements.'], ['Should pupils help review the policy?', 'Yes, pupil voice helps schools test whether reporting routes and language make sense in practice.'], ['Should the policy mention online behaviour?', 'Yes, where online behaviour affects wellbeing or learning in school, it should be addressed clearly.'], ['Which practical resource supports this page?', 'The NI policy checklist provides region-specific review prompts, followed by the staff workshop route.']],
  },
  {
  ...
  },
  {
    ...
    faqs: [['Who is this page for?', 'It is for ROI school leaders, wellbeing teams, SPHE leads and pastoral staff.'], ['Does this replace Department guidance?', 'No. It links to official guidance and supports implementation planning.'], ['Can HIP Psychology support ROI schools?', 'The workshop route covers primary, post-primary and staff enquiries across Ireland.'], ['Which policy resource is designed for ROI schools?', 'The ROI Anti-Bullying Policy Checklist is the region-specific resource.']],
  },
  {
    ...
    faqs: [['Is Cineáltas the same as Bí Cineálta?', 'Cineáltas is the wider action plan; Bí Cineálta provides the procedures for primary and post-primary schools.'], ['Should schools mention Cineáltas in campaign planning?', 'It can help connect classroom activity to wider whole-school priorities.'], ['Where can schools check the current procedures?', 'Use the linked Department of Education and Youth sources for current official information.'], ['What practical resources are available here?', 'Schools can explore the ROI policy checklist and workshop options.']],
  },
  {
    ...
    faqs: [['Is this page primary-specific?', 'Yes, examples and language are aimed at primary school settings.'], ['Which checklist supports this page?', 'The ROI policy checklist provides practical review prompts.'], ['Can classroom resources support Bí Cineálta?', 'Classroom resources can support prevention and pupil understanding when used alongside official procedures.'], ['Does this include child protection advice?', 'No. It links to Tusla for official child protection guidance.']],
  },
  {
    ...
    faqs: [['Is this page post-primary-specific?', 'Yes, examples focus on adolescents, peer groups and online behaviour.'], ['Which workshop route fits this page?', 'Schools can request information about post-primary anti-bullying workshops.'], ['Does it cover cyberbullying?', 'It links to the dedicated cyberbullying hub for deeper detail.'], ['Which checklist supports this page?', 'The ROI policy checklist provides practical review prompts.']],
  },
  {
...
const leadPages = [
  {
    slug: '/resources/free-anti-bullying-classroom-pack/', title: 'Free Anti-Bullying Week Classroom Pack | HIP Psychology', description: 'Explore a planned Anti-Bullying Week classroom pack with assembly scripts, lesson plans, Odd Socks Day printables and parent newsletter support.', eyebrow: 'Free resource', h1: 'Free Anti-Bullying Week classroom pack', lead: 'See what is planned for the primary and post-primary classroom pack: assembly scripts, lesson plans, Odd Socks Day printables and a parent newsletter template.', pack: 'Free Anti-Bullying Week classroom pack', pdf: '/downloads/free-anti-bullying-week-classroom-pack.pdf', bullets: ['2 assembly scripts for primary and post-primary settings', '4 lesson plans for KS1, KS2, KS3 and KS4', 'Odd Socks Day printable activity sheet', 'Parent newsletter template for November planning'],
    sections: [
  ...
  {
    slug: '/resources/anti-bullying-policy-checklist/', title: 'Anti-Bullying Policy Checklist | HIP Psychology', description: 'Explore an anti-bullying policy checklist for NI and ROI schools, aligned to official procedures and practical review questions.', eyebrow: 'Policy resource', h1: 'Anti-bullying policy checklist for school leaders', lead: 'Review anti-bullying policy structure, recording, parent communication and follow-up, then choose the regional checklist for your setting.', pack: 'Anti-bullying policy checklist', pdf: '/downloads/anti-bullying-policy-checklist.pdf', bullets: ['NI policy review prompts for the Addressing Bullying in Schools Act', 'ROI prompts mapped to Bí Cineálta procedures', 'Recording and review schedule prompts', 'Questions for pupil and parent consultation'],
    sections: [
      ...
      { id: 'versions', title: 'NI and ROI versions', html: '<p>Start with the cross-border review questions, then move to the checklist for your setting: <a href="/resources/anti-bullying-policy-checklist-ni/">NI Anti-Bullying Policy Checklist</a> or <a href="/resources/anti-bullying-policy-checklist-roi/">ROI Anti-Bullying Policy Checklist</a>.</p>' },
    ],
  },
  {
    slug: '/resources/anti-bullying-policy-checklist-ni/', title: 'NI Anti-Bullying Policy Checklist | HIP Psychology', description: 'Explore the NI Anti-Bullying Policy Checklist for school leaders reviewing policy against the Addressing Bullying in Schools Act.', eyebrow: 'NI policy resource', h1: 'NI Anti-Bullying Policy Checklist', lead: 'Practical review prompts for Northern Ireland schools, designed to be used alongside the Addressing Bullying in Schools Act and current Education Authority guidance.', pack: 'NI Anti-Bullying Policy Checklist', pdf: '/downloads/ni-anti-bullying-policy-checklist.pdf', bullets: ['Definition and scope prompts', 'Recording and review schedule', 'Parent and pupil consultation prompts', 'Education Authority resource links'],
    sections: [
  ...
  {
    slug: '/resources/anti-bullying-policy-checklist-roi/', title: 'ROI Anti-Bullying Policy Checklist | HIP Psychology', description: 'Explore the ROI Anti-Bullying Policy Checklist for schools reviewing Bí Cineálta procedures and whole-school prevention.', eyebrow: 'ROI policy resource', h1: 'ROI Anti-Bullying Policy Checklist', lead: 'Practical review prompts for schools in the Republic of Ireland, designed to be used alongside current Bí Cineálta procedures and Department guidance.', pack: 'ROI Anti-Bullying Policy Checklist', pdf: '/downloads/roi-anti-bullying-policy-checklist.pdf', bullets: ['Prevention and intervention prompts', 'Recording and oversight schedule', 'Parent communication checks', 'Tusla and Department guidance links'],
    sections: [
...
const resourceHub = {
  slug: '/resources/',
  title: 'Anti-bullying resources for schools | HIP Psychology',
  description: 'Explore Anti-Bullying Week classroom materials and policy-review resources for schools in Northern Ireland and Ireland.',
  h1: 'Anti-bullying resources for school teams',
};

function resourceHubPage() {
  const body = `<section class="section"><p class="eyebrow">School resources</p><div class="content"><h1>${resourceHub.h1}</h1><p class="lead">Choose classroom planning materials or a policy-review checklist for your setting. Every resource is designed to be used alongside current school procedures and official regional guidance.</p></div></section><section class="section tight"><div class="grid two">${leadPages.map((page) => card(page.h1, page.description, page.slug, 'View resource details')).join('')}</div></section>${ctaBlock('workshop')}`;
  return pageChrome(resourceHub, body, [collectionSchema(resourceHub, leadPages)]);
}

function leadPage(page) {
  const sections = (page.sections ?? []).map((section) => `<h2 id="${section.id}">${section.title}</h2>${section.html}`).join('');
  const body = `<section class="section"><p class="eyebrow">${page.eyebrow}</p><div class="content"><h1>${page.h1}</h1><p class="lead">${page.lead}</p><div class="grid two"><article class="card"><h2>What is included</h2><ul>${page.bullets.map((item) => `<li>${item}</li>`).join('')}</ul><p>This resource is being checked against current regional guidance before release.</p></article><article class="card"><h2>Ask for the reviewed resource</h2><p>Leave your work details and we will respond when the reviewed edition is available.</p>${leadForm(page.pack)}</article></div>${sections}<h2>Use it alongside official guidance</h2><p>These practical prompts support school planning. They do not replace current statutory, Department, Education Authority or safeguarding guidance for your setting.</p></div></section>${ctaBlock('workshop')}`;
  return pageChrome(page, body, [articleSchema(page), faqSchema({ faqs: [['Who is this resource for?', 'It is designed for school leaders, pastoral teams, safeguarding leads and educators.'], ['Does it replace official guidance?', 'No. Schools should use the current official guidance and procedures for their setting.'], ['What details does the request form collect?', 'It asks for a name, work email, school or organisation and role.'], ['Can schools ask about workshops from here?', 'Yes. Every resource page links to the workshop page.']] })]);
}

const workshop = {
  slug: '/workshops/anti-bullying/', title: 'Anti-bullying workshops for schools | HIP Psychology', description: 'Anti-bullying workshops for schools in Northern Ireland and Ireland, with primary, post-primary and staff options.', eyebrow: 'School workshops', h1: 'Anti-bullying workshops for schools in Northern Ireland and Ireland', lead: 'Practical, age-appropriate sessions designed to help school communities build shared language, safer responses and clear next steps. Tell us about your school and we will recommend a suitable format.', service: true, links: ['aba', 'ea', 'govBi'],
  sections: [
    ...
    { id: 'school-information', title: 'Information for school teams', html: '<p>Before confirming a booking, school teams can ask for the delivery details, provider information and safeguarding or procurement documents they require. The recommended format and quote will depend on year group, group size, location and delivery requirements.</p>' },
  ],
  faqs: [['Who are workshops for?', 'Schools can enquire about primary pupil, post-primary pupil and staff-team formats.'], ['Can workshops support Anti-Bullying Week?', 'Schools can ask about using a workshop as part of their wider Anti-Bullying Week plan.'], ['Which areas are covered?', 'The enquiry route is for schools in Northern Ireland and the Republic of Ireland.'], ['How is pricing agreed?', 'A quote is based on year group, group size, location, timing and delivery requirements.']],
};

function workshopPage() {
  const schemas = [articleSchema(workshop), faqSchema(workshop), serviceSchema(workshop)];
  const body = `<section class="workshop-hero"><div><p class="eyebrow">${workshop.eyebrow}</p><h1>${workshop.h1}</h1><p class="lead">${workshop.lead}</p><div class="hero-actions"><a class="button" href="#enquiry">Request a workshop quote</a><a class="button secondary" href="#formats">See workshop formats</a></div><p class="form-note">An enquiry does not commit your school to a booking.</p></div><aside class="workshop-summary"><p class="eyebrow">Designed for schools</p><ul><li>Primary pupil sessions</li><li>Post-primary pupil sessions</li><li>Staff-team support</li><li>Whole-school campaign planning</li></ul></aside></section><section class="section"><div class="content">${workshop.sections.map((section) => `<h2 id="${section.id}">${section.title}</h2>${section.html}`).join('')}<h2>What happens next</h2><p>Send the short enquiry form. We will review your year group, group size, location and preferred timing, then reply with suitable options and a clear quote.</p><h2>Useful official resources</h2>${authorityLinks(workshop.links)}</div></section><section class="section form-section" id="enquiry"><div class="content"><p class="eyebrow">Workshop enquiry</p><h2>Request an anti-bullying workshop quote</h2><p>Tell us a little about your school, year group and preferred timing. We will reply with suitable workshop options and the information your team needs to decide next steps.</p>${workshopForm()}</div></section>${ctaBlock('pack')}`;
  return pageChrome(workshop, body, schemas);
}

const supportPage = {
  slug: '/support-and-safeguarding/',
  title: 'Bullying support and safeguarding information | HIP Psychology',
  description: 'Find school, Childline and emergency support routes for bullying concerns in Northern Ireland and the Republic of Ireland.',
  h1: 'Support and safeguarding information',
};

function supportPageHtml() {
  const body = `<section class="section"><p class="eyebrow">Support routes</p><div class="content"><h1>${supportPage.h1}</h1><p class="lead">This website provides school planning information. It is not an incident-reporting service, emergency service or replacement for your school’s safeguarding procedures.</p><h2>For pupils and parents</h2><p>Contact the school’s safeguarding or pastoral team, or speak to a trusted adult. If someone is in immediate danger, call 999 or 112.</p><div class="resource-list"><a class="resource-link" href="https://www.childline.org.uk/get-support/">Childline UK <span>Open</span></a><a class="resource-link" href="https://www.childline.ie/topics/bullying-and-abuse">Childline Ireland <span>Open</span></a></div><h2>For school staff</h2><p>Follow your school’s current safeguarding, child-protection and incident-reporting procedures. Use the official regional sources linked throughout this site when a concern may require escalation.</p><h2>About website enquiries</h2><p>Do not use the workshop or resource forms to report an incident or share a pupil’s name, health information or other confidential case details.</p></div></section>`;
  return pageChrome(supportPage, body, [articleSchema(supportPage)]);
}

const privacyPage = {
  slug: '/privacy/',
  title: 'Privacy notice | HIP Psychology anti-bullying support',
  description: 'Privacy information for workshop and school-resource enquiries made through AntiBullyingNI.co.uk.',
  h1: 'Privacy notice for website enquiries',
};

function privacyPageHtml() {
  const body = `<section class="section"><p class="eyebrow">Privacy</p><div class="content"><h1>${privacyPage.h1}</h1><p class="lead">This notice explains the information requested by the workshop and resource forms on AntiBullyingNI.co.uk.</p><h2>Information requested</h2><p>The forms ask for professional contact information such as your name, work email, school or organisation, role and details needed to understand a workshop or resource request.</p><h2>How enquiries are used</h2><p>Information submitted through these forms is used to respond to the request and arrange relevant follow-up. The forms are not intended for confidential pupil information or incident reports.</p><h2>Data minimisation</h2><p>Please do not include pupil names, health information, safeguarding records or other confidential case details. Use your school’s approved safeguarding and reporting routes for individual concerns.</p><h2>Retention, sharing and rights</h2><p>Enquiry information should be kept only for as long as it is needed to respond and meet relevant record-keeping obligations. To ask about access, correction or deletion of your information, use the <a href="https://hippsychology.com/contact-us/">HIP Psychology contact page</a>.</p></div></section>`;
  return pageChrome(privacyPage, body, [articleSchema(privacyPage)]);
}
...
const generatedPages = [home, ...pages, blogHub, ...blogPosts, workshop, resourceHub, ...leadPages, supportPage, privacyPage];
...
writeFileForSlug(workshop.slug, workshopPage());
writeFileForSlug(resourceHub.slug, resourceHubPage());
leadPages.forEach((page) => writeFileForSlug(page.slug, leadPage(page)));
writeFileForSlug(supportPage.slug, supportPageHtml());
writeFileForSlug(privacyPage.slug, privacyPageHtml());
writeFileForSlug('/anti-bullying-week/', redirectPage('/anti-bullying-week/2026/', 'the Anti-Bullying Week 2026 guide'));
writeFileForSlug('/cyberbullying/', redirectPage('/cyberbullying-in-schools/', 'cyberbullying guidance for schools'));
writeFileForSlug('/school-workshops/', redirectPage('/workshops/anti-bullying/', 'anti-bullying workshops for schools'));

writeSitemaps(generatedPages);

const report = generatedPages.map((page) => `${page.slug} ${page.h1 || page.title || ''}`).join('\n');
fs.writeFileSync('phase-1-routes.txt', `${report}\n`);
console.log(`Generated ${generatedPages.length} routes. Resource PDFs are maintained separately and require approval before release.`);
