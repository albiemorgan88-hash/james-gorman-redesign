...
interface GuidePageTemplateProps {
  ...
  practicalTakeawayTitle?: string;
  practicalTakeawayText?: string;
}

const defaultSidebarPoints = [
  "Start with one workflow, one owner, and one metric.",
  "Keep human approvals where the downside of mistakes is meaningful.",
  "Use the pilot to prove value before you scale tooling or spend.",
];

const takeawayCards = [
  {
    title: "Start narrow",
    text: "One painful workflow will teach you more than a broad vague transformation plan.",
  },
  {
    title: "Protect approvals",
    text: "Keep the human in the loop wherever risk, regulation, or brand trust matters.",
  },
  {
    title: "Measure honestly",
    text: "Track time saved, response speed, error reduction, or conversion uplift with a real baseline.",
  },
];
...
export default function GuidePageTemplate({
  ...
  sidebarPoints = defaultSidebarPoints,
  assessmentTitle = (
    <>
      Ready to
      <br />
  ...
  assessmentDescription = "Blue Canvas will review your workflow, show where AI can create leverage, and give you a straight answer on what is worth automating now.",
  contactTitle = "Free AI Agent Assessment",
  contactDescription = "Tell us about the workflow you want to improve",
  ...
  return (
    <>
      <section className="hero-gradient relative overflow-hidden min-h-[70vh] flex items-center grain">
        ...
        <div
          className="absolute bottom-20 left-[5%] w-[300px] h-[300px] bg-blue/10 rounded-full blur-[100px] glow-pulse"
          ...
          <div className="space-y-8">
            {sections.map((section, index) => (
              <article key={section.id} id={section.id} className="bg-white rounded-3xl border border-border-light shadow-card p-8 md:p-10">
                ...
                <div
                  className="prose prose-lg max-w-none prose-p:text-muted-dark prose-p:leading-relaxed prose-li:text-muted-dark prose-strong:text-dark prose-a:text-orange prose-a:no-underline hover:prose-a:underline"
          ...
          <aside className="lg:sticky lg:top-24 space-y-6">
            ...
            <div className="bg-white rounded-3xl p-8 border border-border-light shadow-card">
              ...
              <p className="text-muted-dark leading-relaxed mb-4">
                They want a grounded AI workflow plan, not a stack of hype. Phil Patterson focuses on practical automation, clear guardrails, and return on effort.
              </p>
              <a
                href="#assessment"
                className="inline-flex items-center justify-center bg-orange text-white px-6 py-3 rounded-xl font-semibold hover:bg-orange-hover transition-all"
              >
                Book an assessment
              </a>
            </div>
      ...
      <section className="bg-white py-24">
        <div className="max-w-[1140px] mx-auto px-6">
          <div className="max-w-[820px] mb-12">
            <h2 className="font-heading text-3xl md:text-4xl font-bold text-dark mb-4">{practicalTakeawayTitle}</h2>
            <p className="text-muted-dark text-lg leading-relaxed">{practicalTakeawayText}</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {takeawayCards.map((card) => (
              <div key={card.title} className="bg-light rounded-2xl p-6 border border-border-light">
          ...
          <div className="space-y-6">
            {faqs.map((faq, index) => (
              <div key={index} className="bg-white rounded-2xl p-8 border border-border-light shadow-sm">
                ...
                <p className="text-muted-dark leading-relaxed" dangerouslySetInnerHTML={{ __html: faq.answer }} />
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="assessment" className="bg-navy text-white py-24 relative overflow-hidden grain">
        <div className="max-w-[1140px] mx-auto px-6 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
            <div>
              <h2 className="font-heading text-3xl md:text-4xl font-bold mb-6">{assessmentTitle}</h2>
              <p className="text-white/70 text-lg leading-relaxed mb-8">{assessmentDescription}</p>
            ...
            <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-8">
              <h3 className="font-heading font-bold text-xl text-white mb-1">{contactTitle}</h3>
              <p className="text-white/60 text-sm mb-6">{contactDescription}</p>
              <ContactForm />
            </div>
          </div>
        </div>
