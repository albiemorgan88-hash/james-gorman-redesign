...
(async()=>{
  ...
  ws.onopen=async()=>{
    ...
    const expr=`Array.from(document.querySelectorAll('a,button,[role="link"],[role="button"]')).map(el=>({text:(el.innerText||el.textContent||'').trim(), href:el.href||el.getAttribute('href')||'', aria:el.getAttribute('aria-label')||'', tag:el.tagName})).filter(x=>x.text||x.href).slice(0,120)`;
