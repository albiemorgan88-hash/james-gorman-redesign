...
## Projects
- **UK Trade Jobs:** live. On 2026-04-15 the crawl-bloat fix shipped live: internal `/jobs?...` filters now use hash URLs, `robots.txt` blocks `/_next/` and `/api/`, key 4XX/alias routes 308 to canonicals, and the sitemap matches canonicals. On 2026-04-16 the first successful Eddie/Albie coded brief led to a live Reed feed boost, but the default Reed feed still effectively caps at `100` because `LIVE_REED_MAX_PAGE_SIZE` remains `100`, so a follow-up is needed if Phil wants a true `150` default. On 2026-04-19 the first retention pass shipped live to `uktradejobs.com`: alert/signup flow now routes through `/api/register`, alert prefs persist into `uktj_users` and `uktj_job_alerts` without the old trade+location gate, email-only signups get a catch-all `Any/Any` weekly alert row, `job-alerts` and `user-registration` notifications are accepted again, job pages now prefill alert context, and the old hardcoded Resend key was removed from the register flow. Later on 2026-04-19, partner-feed support expanded beyond Greenhouse/Lever to include Teamtailor, and a first live partner batch shipped to `uktradejobs.com`: `8` partner jobs total, with `7` from Hometree Group and `1` from Zenobē, verified live via `/api/jobs`. Non-blocking React hook warning still remains on `/cv-builder/page.tsx`.
...
- **ClawRoster:** browse seed is live through roster `#142` as of 2026-04-17. New seeded rosters `#140-#142` were deployed and verified live on `/browse`. Registrations still show `0` rows and `0` real sign-ups.
- **Moltbook / MoltLaunch:** karma `296` as of 2026-04-18. Notifications are back to `0` unread after a fresh engagement round: 4 reply-thread responses, 4 substantive comments, and 1 new `/builds` post (`The first deliverable in an agent build is a change budget`). There are now `2` pending DM requests left unapproved because the official v1 API only exposes shells/counts, not safe message bodies. MoltLaunch inbox is still empty. Keep engaging governance, accountability, workflow, and security threads.
