#!/usr/bin/env python3
"""Comprehensive Google Ads Performance Report for Blue Canvas."""
    ...
    if rows:
        for i, row in enumerate(rows, 1):
            ...
            if descs:
                ...
                for d in descs:
                    pinned = f" (pinned: {d.pinned_field.name})" if d.pinned_field and d.pinned_field.name != "UNSPECIFIED" else ""
...
def section_conversions(client):
    ...
    try:
        rows = run_query(client, q)
        if rows:
            lines.append("| Conversion Action | Type | Status | Category | Counting | Conversions |")
            lines.append("|-------------------|------|--------|----------|----------|-------------|")
            for row in rows:
                ca = row.conversion_action
                m = row.metrics
                lines.append(f"| {ca.name} | {ca.type_.name} | {ca.status.name} | {ca.category.name} | {ca.counting_type.name} | {m.conversions:.0f} |")
            lines.append("")
        else:
...
def section_campaign_settings(client):
    ...
    for row in rows:
        ...
        lines.append(f"- **Start Date:** {c.start_date}")
        lines.append(f"- **End Date:** {c.end_date if c.end_date else 'None (ongoing)'}")
    ...
    # Device performance (not targeting, but performance by device)
...
def section_recommendations(all_data):
    ...
    lines.append("- If CPC is high, consider adjusting bid strategy or targeting")
    lines.append("")
    lines.append("### Conversion Tracking")
    lines.append("- **CRITICAL:** If no conversion tracking → set up immediately. Without it, Google can't optimize.")
    lines.append("- Minimum: track contact form submissions and phone calls")
    lines.append("- Add a Google Ads tag to the thank-you/confirmation page")
    lines.append("")
    lines.append("### Structure")
    lines.append("- Consider SKAGs (Single Keyword Ad Groups) for top performers")
    lines.append("- Ensure each ad group has 3+ responsive search ads for testing")
    lines.append("- Use ad extensions: sitelinks, callouts, structured snippets, call extension")
    lines.append("")
    lines.append("### Device & Location")
    lines.append("- Review device performance — add bid adjustments for underperforming devices")
    lines.append("- Verify location targeting covers your actual service area")
    lines.append("- Consider location bid adjustments for high-performing areas")
    lines.append("")
...
def main():
    ...
    report_sections.append(f"**Account:** 801-851-4760 | **Manager:** 396-378-6948\n")
    ...
    # Recommendations (static analysis)
    print("Generating recommendations...")
    report_sections.append(section_recommendations(None))
